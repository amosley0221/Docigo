...
interface AssignModalProps {
  ...
  onClose: () => void;
  onAssign: (target: { locationId: string; groupId: string }) => void;
}

export function AssignModal({ open, pending, onClose, onAssign }: AssignModalProps) {
  ...
  const title =
    pending.kind === 'file' ? 'Organize this file' : 'Save this quote';
  const subtitle =
    pending.kind === 'file'
        ...
        <>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!gid) return;
              onAssign({ locationId, groupId: gid });
            }}
          >
            <Icon name="check" width={14} height={14} />
            {pending.kind === 'file' ? 'Save file' : 'Save quote'}
          </button>
        </>
      }
    >
